...
<?php
// $sql = "SELECT transaksi.no_transaksi,transaksi.id_customer,transaksi.tanggal as tanggal_trans,transaksi.rekening_pengirim,transaksi.bukti_pembayaran,transaksi.status_pembayaran,transaksi.status_pemesanan,layanan.nama,detail_transaksi.tanggal,cart.status,user.nama as nama_terapis FROM transaksi JOIN detail_transaksi ON transaksi.no_transaksi=detail_transaksi.no_transaksi JOIN cart ON detail_transaksi.id_cart=cart.id_chart JOIN layanan ON cart.id_detail_layanan=layanan.id_layanan JOIN detail_layanan ON detail_layanan.id_detailLayanan=cart.id_detail_layanan JOIN terapis ON detail_layanan.id_terapis=terapis.id_terapis JOIN user ON user.id_user=terapis.id_user WHERE transaksi.id_customer=$id_user";
